...
spec = do
  describe "Strom" do
    ...
    describe "Transformations" do
      ...
      it "bind (>>=) preserves order with multi-chunk source" do
        -- rangeStrom 1 2001 produces two 1000-element chunks, exercising the Loop branch
        result <- runOm $
          Strom.rangeStrom 1 2001
            >>= (\n -> Strom.succeed (n * 2))
            # Strom.takeStrom 2005
            # Strom.runCollect
        result `shouldEqual` (Array.range 1 2000 # map (_ * 2))
    ...
    describe "Combining" do
      ...
      it "zip handles mismatched chunk sizes (multi-chunk vs single-chunk)" do
        -- Regression: previously dropped elements when one stream had more chunks
        result <- runOm $
          Strom.zipStrom
            (Strom.rangeStrom 1 2001)  -- two 1000-element chunks
            (Strom.fromArray [ 10, 20, 30 ])  -- single chunk
            # Strom.runCollect
        result `shouldEqual` [ Tuple 1 10, Tuple 2 20, Tuple 3 30 ]

      it "interleave alternates chunks from two streams" do
        result <- runOm $
          Strom.interleave
            (Strom.fromArray [ 1, 3, 5 ])
            (Strom.fromArray [ 2, 4, 6 ])
            # Strom.runCollect
        -- fromArray produces single chunks, so interleave emits first chunk then second
        result `shouldEqual` [ 1, 3, 5, 2, 4, 6 ]

      it "interleave terminates with finite streams" do
        -- Regression: previously infinite looped because Done(Just) streams were re-pulled
        result <- runOm $
          Strom.interleave
            (Strom.succeed 1)
            (Strom.succeed 2)
    ...
    describe "Grouping" do
      ...
      it "grouped preserves all elements with multi-chunk source" do
        -- rangeStrom 1 2001 produces two 1000-element chunks.
        -- groupedStrom 500 means chunk boundaries align with group size
        -- (1000 is a multiple of 500), which previously dropped complete last groups.
        result <- runOm $
          Strom.rangeStrom 1 2001
            # Strom.groupedStrom 500
            # Strom.mapStrom Array.length
            # Strom.runCollect
        result `shouldEqual` [ 500, 500, 500, 500 ]
    ...
    describe "Cancellation" do
      ...
      it "cancelStrom on simple stream is no-op" do
        liftEffect $ Strom.cancelStrom (Strom.fromArray [ 1, 2, 3 ] :: Strom.Strom {} () Int)

      it "takeStrom + bracket releases resource" do
        ref <- liftEffect $ Ref.new false
        let
          strom = Strom.bracket
            (pure unit)
            (\_ -> liftEffect $ Ref.write true ref)
            (\_ -> Strom.rangeStrom 1 1000)
        result <- runOm $ Strom.runCollect (Strom.takeStrom 5 strom)
        result `shouldEqual` [ 1, 2, 3, 4, 5 ]
        released <- liftEffect $ Ref.read ref
        released `shouldEqual` true

      it "takeWhileStrom + bracket releases resource" do
        ref <- liftEffect $ Ref.new false
        let
          strom = Strom.bracket
            (pure unit)
            (\_ -> liftEffect $ Ref.write true ref)
            (\_ -> Strom.rangeStrom 1 1000)
        result <- runOm $ Strom.runCollect (Strom.takeWhileStrom (_ < 4) strom)
        result `shouldEqual` [ 1, 2, 3 ]
        released <- liftEffect $ Ref.read ref
        released `shouldEqual` true

      it "bracket releases resource on natural multi-chunk completion" do
        ref <- liftEffect $ Ref.new false
        let
          -- rangeStrom 1 2001 produces two chunks (>1000 elements), so bracket sees Loop first
          strom = Strom.bracket
            (pure unit)
            (\_ -> liftEffect $ Ref.write true ref)
            (\_ -> Strom.rangeStrom 1 2001)
        result <- runOm $ Strom.runCollect strom
        Array.length result `shouldEqual` 2000
        released <- liftEffect $ Ref.read ref
        released `shouldEqual` true

      it "takeUntilStrom + bracket releases resource" do
        ref <- liftEffect $ Ref.new false
        let
          strom = Strom.bracket
            (pure unit)
            (\_ -> liftEffect $ Ref.write true ref)
            (\_ -> Strom.rangeStrom 1 1000)
        result <- runOm $ Strom.runCollect (Strom.takeUntilStrom (_ == 5) strom)
        result `shouldEqual` [ 1, 2, 3, 4, 5 ]
        released <- liftEffect $ Ref.read ref
        released `shouldEqual` true
